...
resource "azurerm_virtual_machine" "k8s" {
  ...
  storage_data_disk {
    name            = azurerm_managed_disk.k8s[count.index].name
    managed_disk_id = azurerm_managed_disk.k8s[count.index].id
    create_option   = "Attach"
    lun             = 0
    disk_size_gb    = azurerm_managed_disk.k8s[count.index].disk_size_gb
  }
